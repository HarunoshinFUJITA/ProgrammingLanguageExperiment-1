...
void select_cards_free(int select_cards[8][15], int my_cards[8][15], state *field_status){
  ...
  if(count_cards(select_cards)==0)
    search_low_sequence(select_cards, info_table); // 最弱の階段を提出

  if(count_cards(select_cards)==0)
    search_low_pair(select_cards, info_table, my_cards); // 最弱のペアを提出
...
void select_cards_restrict(int select_cards[8][15], int my_cards[8][15], state *field_status){
  ...
  if(field_status->is_sequence==1){ // 場が階段のとき
    if(field_status->is_lock==1){ // 場が縛られている
      remove_suit(info_table, field_status->suit, 1);
      remove_low_card(info_table, field_status->order, 0);
    ...
    if(field_status->is_lock==1){ // 場が縛られている
      remove_suit(info_table, field_status->suit, 1);
      remove_low_card(info_table, field_status->order, 0);
      search_low_pair_restrict(select_cards, info_table, my_cards, field_status->quantity);
    }else{ // 場が縛られていない
      remove_low_card(info_table, field_status->order, 0);
      search_low_pair_restrict(select_cards, info_table, my_cards, field_status->quantity);
    }
